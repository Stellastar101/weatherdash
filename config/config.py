...
sites_data_cfg = Config.configure_data_node(id="sites_data")
weather_data_cfg = Config.configure_data_node(id="weather_data")

site_names_cfg = Config.configure_data_node(id="site_names")
dates_cfg = Config.configure_data_node(id="dates")
selected_sites_cfg = Config.configure_data_node(id="selected_sites")
selected_aws_cfg = Config.configure_data_node(id="selected_aws", default_data=[])
...
def filter_data_task(sites_data, weather_data, selected_sites, selected_aws, dates, parameters):
    ...
    if selected_sites:
        ...
        filtered = weather_data[weather_data['AWS ID'].isin(aws_list)]
    else:
    ...
    return filtered[['date', 'AWS ID'] + parameters]
...
def create_figure_task(filtered_data, parameters):
    ...
    fig.update_layout(title="Weather Parameters Over Time", xaxis_title="Date", yaxis_title="Value", legend_title="AWS & Parameter")
    return fig
