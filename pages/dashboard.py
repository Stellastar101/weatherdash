...
from scenarios.pipeline import create_scenario
...
def on_site_selection(state):
    ...
    state.scenario.selected_sites.write(state.site_names if state.site_names else None)
    
    # Update AWS list based on selected sites
    sites_data = state.scenario.sites_data.read()
    if state.site_names:
        aws_list = sites_data[sites_data['Sites'].isin(state.site_names)]['AWS'].tolist()
        state.Aws = aws_list
...
def on_aws_selection(state):
    ...
    state.scenario.selected_aws.write(
        state.Aws_default if isinstance(state.Aws_default, list) else [state.Aws_default]
    )
    state.scenario.submit()
    state.figure = state.scenario.figure.read()
...
with tgb.Page() as page:
    with tgb.part("text-center"):
        ...
        tgb.selector(
            label="Parameters",
            lov=["temperature", "pressure", "gust", "wind"],
            value="{parameters}",
            dropdown=True,
            multiple=True,
...
def on_site_selection(state):
    ...
    sites_data = state.scenario.sites_data.read()
    if state.site_names:
        # Make sure we're looking for AWS ID column
        aws_list = sites_data[sites_data['Sites'].isin(state.site_names)]['AWS ID'].tolist()
        state.Aws = aws_list
        state.Aws_default = aws_list[0] if aws_list else None
    else:
        state.Aws = []
