...
from taipy.core.core import Core
import sys
import os
import pandas as pd
# Ensure the parent directory is in the system path
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from config.config import scenario_cfg, sites_df, weather_df
scenario = Core.create_scenario(scenario_cfg)
def create_scenario():
...
from taipy import Config, create_scenario as taipy_create_scenario, run

def create_scenario_instance():
    run()
    scenario = taipy_create_scenario(scenario_cfg)
    scenario.selected_sites.write(None)
    scenario.selected_aws.write([])
    scenario.parameters.write(["temperature_air_20_seconds"])
    return scenario
